...
const Login = () => {
  ...
  const googleLogin = async () => {
    ...
    try {
      ...
      toast.success("User Logged in with Google!", { position: 'top-center' });
      navigate('/');
    } catch (error) {
      console.log(error.message);
      toast.error(error.message, { position: 'top-center' });
  ...
  return (
    <div className="bg-grey-lighter min-h-[40rem] flex flex-col">
      <div className="container max-w-sm mx-auto flex-1 flex flex-col items-center justify-center px-2">
        <div className="bg-white px-6 py-8 rounded text-black w-full shadow-xl border-2 border-gray-200">
          <form onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              className="w-full font-semibold text-center py-3 rounded bg-indigo-600 text-white hover:bg-indigo-400 focus:outline-none my-1"
            >
              Log In
            </button>
        ...
        <div className="text-grey-dark mt-6 font-semibold flex gap-1">
          Need an account?
          <Link className="no-underline font-semibold text-blue-500" to="/signup">
            Create an account
          </Link>.
        </div>
      </div>
